...
class AnimationController {
    ...
    private readonly numberOfStars = 5500; // Optimized for performance
    ...
    public render() {
        ...
        ctx.fillStyle = '#08090b'; // Consistent with SaaS background
...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            let easing = disp < 0.3 ? controller.lerp(disp, Math.pow(disp, 2), disp/0.3) : disp < 0.7 ? controller.lerp(Math.pow(disp, 2), controller.easeOutElastic(disp), (disp-0.3)/0.4) : controller.easeOutElastic(disp);
            ...
            if (disp < 0.3) {
                ...
                sx = controller.lerp(spiralPos.x + this.dx * 0.3, spiralPos.x + this.dx * 0.7, mp) + px * mp;
                sy = controller.lerp(spiralPos.y + this.dy * 0.3, spiralPos.y + this.dy * 0.7, mp) + py * mp;
            } else {
            ...
            const position = new Vector3D((this.z - controller['cameraZ']) * sx / 100, (this.z - controller['cameraZ']) * sy / 100, this.z);
            const ds = 8.5 * this.strokeWeightFactor * (disp < 0.6 ? 1.0 + disp * 0.2 : 1.2 * (1.0 - (disp-0.6)/0.4) + this.finalScale * ((disp-0.6)/0.4));
            controller.showProjectedDot(position, ds);
        }
    }
}
...
export function SpiralAnimation() {
    ...
    useEffect(() => {
        ...
        const s = Math.max(dim.w, dim.h);
        c.width = s * dpr;
        c.height = s * dpr;
        c.style.width = `${dim.w}px`;
        c.style.height = `${dim.h}px`;
        ctx.scale(dpr, dpr);
        animationRef.current = new AnimationController(c, ctx, dpr, s);
